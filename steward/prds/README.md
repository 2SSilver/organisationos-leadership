...
Run 2026-09-18, from `steward/prds/`: `coverage OK: 167 files claimed, 0 unclaimed`. (161 on 2026-09-08; +2 for `banned-scan.sh`/`banned-scan.test.sh`, added 2026-09-14 closing the `!context-near` finding; +4 for this wave's `.github/scripts/format-scan.sh`, `format-scan.test.sh`, `setup-check.sh`, `setup-check.test.sh`, added to PRD-10's `specifies:` list.)
...
| [PRD-04](prd-04-confidentiality.md) | Confidentiality & Back-Flow Boundary | 1 | Let external work inform shared knowledge without ever making the client identifiable, by default rather than by contributor vigilance. | 4 ENFORCED · 3 SHIPPED · 1 CONVENTION · 1 GAP |
...
| [PRD-13](prd-13-permissions-supply-chain.md) | Agent Permissions & Supply Chain | 3 | Let an agent session read the whole harness but change only the surface it belongs to, with nothing executable un-pinned. | 4 SHIPPED · 3 CONVENTION |
...
| [PRD-16](prd-16-adoption-onboarding.md) | Adoption & Onboarding | 4 | Let a stranger stand the whole harness up, and a new joiner become productive in it, from the three published repositories alone. | 6 SHIPPED · 2 CONVENTION |
...
Across the set: 22 ENFORCED · 42 SHIPPED · 42 CONVENTION · 1 GAP, out of 107 requirements total. (2026-09-18, reflecting findings N1, N2 and N5 closed 2026-09-15: FR-05.2 moved `ENFORCED (local)` → `ENFORCED (CI)`, no change to this count; FR-13.4 and FR-16.4 each record their placeholder-substitution half moving `GAP` → `CONVENTION`. Per the counting convention above, a compound status is counted under its first-named half, and that half now leads in both requirements' `Status:` lines — so the counted primary status for each also reads `SHIPPED` → `CONVENTION`. Neither requirement's previously-`SHIPPED` half regressed; only the substitution half closed.)

Read on its own, that "1 GAP" understates what this set found by more than an order of magnitude. Only one requirement, in PRD-04, carries `Status: GAP`; that status marks a requirement whose entire logic is unbuilt. But every PRD also carries a section 8 (Known gaps & open questions), and across the set that section holds 29 findings marked `GAP`, spread across 15 of the 16 documents (every one except PRD-05). Most of the harness's gaps are not missing mechanisms; they are mechanisms that exist and were built for a real purpose, but fall short of it in some specific, documented way: a check that has never rejected anything in real CI, a ban enforced by reviewer judgement with no field for a reviewer to check against, a setup path that leaves a reference unresolved. A reader who wants the full gap picture reads section 8 of every PRD, not the `Status:` line count alone.
...
- **`!context-near` co-occurrence detection** was declared in `banned-patterns.yml` and named twice in `coverage-gaps.md` as the defence against two anodyne facts combining to identify someone, but the check's extraction read only each entry's `.pattern` key, so a `!context-near` entry was dropped before the scan ever ran. Closed 2026-09-14 (Foundation v1.1.3): the scan selects on the YAML tag, both enforcement points call one implementation, and a fixture suite runs on every Foundation PR. The declared `match:` modifiers were removed rather than implemented, so the file no longer claims matching behaviour the scan does not apply ([PRD-04](prd-04-confidentiality.md)).
...
- **Templated repositories got no tags — closed 2026-09-15.** `gh repo create --template` copies no tags from the template repository, and 21 distinct `@v1`-pinned callers across Leadership (10) and Domain (11) resolved against nothing on a freshly templated Foundation. `setup-org.md`'s Step 5 now opens with `git tag v1 && git push origin v1`, run before that same step enables Actions, so a literal first run of the documented path no longer leaves any caller unresolved (Foundation PR #11, `5aea4e5`). This is `CONVENTION`, not `ENFORCED`: Step 5 is a step an adopter can still skip, and nothing in CI compels running it ([PRD-10](prd-10-ci-architecture.md) FR-10.2, [PRD-16](prd-16-adoption-onboarding.md) FR-16.4).
- **Unsubstituted supply-chain pins shipped live — closed 2026-09-15.** `REPLACE-WITH-AUDITED-COMMIT-SHA` and `<pinned-version-or-ref>` still ship in all three repositories' `.claude/settings.json`, but `setup-org.md` now names both directly under "Also in Step 3 — pin the plugin supply chain," and a new Step 10 runs `.github/scripts/setup-check.sh`, which fails if either placeholder remains. Also `CONVENTION`, not `ENFORCED`: `setup-check.sh` is adopter-run, gates nothing, and executes on a workstation before any CI exists to check it — an adopter who completes Step 3 but skips Step 10's verification still finishes with both pins unsubstituted ([PRD-13](prd-13-permissions-supply-chain.md) FR-13.4, [PRD-16](prd-16-adoption-onboarding.md) FR-16.4).
